function Parallel_Coords(container, data, initialState) {
	...
	// var margin = {top: 100, right: 20, bottom: 10, left: 40};
	// var width = 900 - margin.left - margin.right;
	// var height = 825 - margin.top - margin.bottom;
    ...
    this.update = function(data, targetState){
	        svg.selectAll("*").remove();

		var filteredData = data.filter(function(d) {
			if (d["Geographic Area"] === targetState)
				return d;
		});
        ...
        var foreground = svg.append("g")
            ...
            .append("path")
            .attr("d", path)
            .style("opacity", +((total-ftotal)/total)*0.1 );
